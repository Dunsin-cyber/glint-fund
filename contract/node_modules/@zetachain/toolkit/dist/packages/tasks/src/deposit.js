...
const main = async (args, hre) => {
    ...
    const refundFeeAmount = ethers.utils.formatUnits(refundFee.amount, refundFee.decimals);
    let decimals = 18;
    if (erc20) {
        const contract = new ethers.Contract(erc20, ERC20_json_1.default.abi, signer);
    ...
    if (args.message) {
        try {
            ...
            if (Array.isArray(msg) &&
                msg.length === 2 &&
                Array.isArray(msg[0]) &&
                msg[0].every((item) => typeof item === "string") &&
                msg[1].every((item) => typeof item === "string")) {
    ...
    if (!args.ignoreChecks) {
        if (chain === "zeta_testnet" || chain === "zeta_mainnet") {
            throw new Error("the --network defines the chain from which the deposit will be made. The --network value cannot be zeta_testnet or zeta_mainnet");
        }
        if (args.recipient) {
        ...
        if (message) {
            if (!args.recipient) {
                throw new Error("--recipient is not specified. Please, provide a valid omnichain contract address on ZetaChain");
            }
            const rpc = client.getEndpoint("evm", "zeta_testnet");
            const provider = new ethers.providers.StaticJsonRpcProvider(rpc);
            const code = await provider.getCode(args.recipient);
            if (code === "0x") {
                throw new Error("seems like --recipient is an EOA and not a contract on ZetaChain. At the same time the --message is not empty, which indicates this is a 'deposit and call' operation. Please, provide a valid omnichain contract address as the --recipient value");
            }
        }
    }
    ...
    if (erc20) {
        ...
        symbol = coins.find((c) => c.coin_type === "Gas" && c.foreign_chain_id === chainId)?.symbol;
    }
    if (args.json) {
...
exports.depositTask = (0, config_1.task)("deposit", "Deposit native gas or ERC-20 assets to ZetaChain as ZRC-20.", main)
